...
//[바디_선택하기 누름] 토글아이템 클릭했을 때 -> 지원자 선택(등록)
//private val context: Context, 일단 뺌
class AdapterRecruitApplicants4(private val context: Context, private val listData: ArrayList<ApplicantInfoData>, private val returnRegistCount: (Int) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        fun bind(data: ApplicantInfoData, position: Int){
            ...
            binding.chkChoose.setOnClickListener {
                ...
                if(binding.chkChoose.isChecked){
                    registList.add(data.co_portfolioId)
                }else{
                    registList.remove(data.co_portfolioId)
                }
                registCounter()
