...
import com.codev.android.databinding.RecycleChatRoomListBinding
...
class AdapterChatRoomList(private val context: Context, private val listData: ArrayList<com.codev.android.ResponseOfGetChatRoomListData>, private val returnToActivity: (Int, Int) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    fun findRoomId(data: com.codev.android.ResponseOfGetChatRoomListData){
        ...
        if (index != -1){
            ...
            val temp = com.codev.android.ResponseOfGetChatRoomListData(
                data.roomId,
                data.room_type,
                data.room_title,
                data.mainImg,
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(holder){
            is com.codev.android.AdapterChatRoomList.ChatRoomListViewHolder -> {
    ...
    inner class ChatRoomListViewHolder(private val binding: RecycleChatRoomListBinding): RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(data: com.codev.android.ResponseOfGetChatRoomListData, position: Int){
            ...
            binding.room.setOnClickListener {
                com.codev.android.ChatClient.exit()
                com.codev.android.ChatClient.moveChatRoom(
                    itemView.context,
                    data.room_type,
                    data.roomId,
                    title,
                    binding.roomMemberNumber.text.toString().toInt(),
                    data.isRead
                )
            }
        }
