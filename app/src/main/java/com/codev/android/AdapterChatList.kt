...
import com.codev.android.databinding.RecycleChatItemDateBinding
import com.codev.android.databinding.RecycleChatItemInviteBinding
import com.codev.android.databinding.RecycleChatItemMyContinueBinding
import com.codev.android.databinding.RecycleChatItemMyFirstBinding
import com.codev.android.databinding.RecycleChatItemOtherContinueBinding
import com.codev.android.databinding.RecycleChatItemOtherFirstBinding
...
class AdapterChatList(private val context: Context, private val listData: ArrayList<com.codev.android.ResponseOfGetChatListData>, private val returnDataListSize: (Int) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private val MY = 1
    ...
    fun addData(data: com.codev.android.ResponseOfGetChatListData){
...
//    "EXIT", "INVITE", "LEAVE", "TALK"(분류해서 내거, 상대방거, 연속 유무), "ENTER"
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(holder){
            is com.codev.android.AdapterChatList.ChatOtherFirstViewHolder -> {
                holder.bind(listData[position], position)
            }
            is com.codev.android.AdapterChatList.ChatMyFirstViewHolder -> {
                holder.bind(listData[position], position)
            }
            is com.codev.android.AdapterChatList.ChatOtherContinueViewHolder -> {
                holder.bind(listData[position], position)
            }
            is com.codev.android.AdapterChatList.ChatMyContinueViewHolder -> {
                holder.bind(listData[position], position)
            }
            is com.codev.android.AdapterChatList.ChatDayViewHolder -> {
                holder.bind(listData[position], position)
            }
            is com.codev.android.AdapterChatList.ChatInviteViewHolder -> {
                holder.bind(listData[position], position)
            }
        }
    }
    ...
    override fun getItemViewType(position: Int): Int {
        if (listData[position].type == "DAY"){
            ...
            return INVITE
        }else if (com.codev.android.UserSharedPreferences.getKey() == listData[position].sender) {
            if(position == 0){
                return MY
    ...
    inner class ChatOtherFirstViewHolder(private val binding: RecycleChatItemOtherFirstBinding): RecyclerView.ViewHolder(binding.root){
        fun bind(data: com.codev.android.ResponseOfGetChatListData, position: Int){
